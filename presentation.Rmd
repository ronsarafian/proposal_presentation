...
subtitle: "Geographical Applications of Modern Statistical Learning Algorithms"
author: Ron sarafian
...
### Observational study
 ...
 - **Requires:** Accurate data of air pollution concentration levels and approproate health data

 - **Exists:** Air pollution (PM) monitoring stations measurments (Spatially limited)
...
*PM: Microscopic solid or liquid matter suspended in Earth's atmosphere. They have impacts on climate and precipitation that adversely affect human health.*
 ...
 - **Difficulty:** Monitoring station limited spatial coverage
 - **Solution:** Predict PM to units (e.g. spatial units) without measurments
...
## How to generate PM data (1):

### <span style="color:darkred"> **Interpolation** methods </span> 
...
### <span style="color:darkred"> Using external **Spatial** information </span> 
...
## How to generate PM data (2):

### <span style="color:darkred"> Using external **Spatio-temporal** information </span>

Advantages: **High accuracy, Unlimited coverage (space and time)**

- **Aerosol Optical Depth (AOD)** is associated with **ground PM measurements** in different geographical areas
- therefore considered as a **good spatio-temporal predictor**.

<div class="columns-2">

   <iframe width="50" src="https://svs.gsfc.nasa.gov/vis/a010000/a012300/a012302/MODIS_Aerosol_Optical_Depth_large.mp4" frameborder="0" allowfullscreen>
   </iframe>
   
   <span style="color:darkred"> *AOD: measures the light extinction by aerosol scattering and its absorption in the atmospheric column*
</span>
...
### 1. Investigate the Geographic model preformance estimation procedure
 ...
 - Recognize the **epidemiological study goals**
 - Avoid **overfitting**

### 2. Improve prediction performance by modeling the dependence structure

 - Use **Generalized least squares (GLS)**
 - Learn the errors **Covariance structure**

### 3. Fit the Geographic model when the data is very big

 - Reduce the problem of model fitting by **applying GLS**
 - Employ **sparse representations**, **memory efficiency** and **parallel computing** tools
...
Today, PM model performance are typically estimated using <span style="color:darkred"> **Cross-Validation** </span>. i.e:
...
1. The model ultimate goal is not PM prediction accuracy, but the the **epidemiological results reliability**.
    + <span style="color:darkred"> The **Loss Function** has to recognize this goal! </span>
2. When the training and validationa sets are **correlated**, CV is likely to **overfit**.
    + <span style="color:darkred"> An appropriate **Resampling Scheme** is needed! </span>



## Performance Estimation - The Loss Function

### Illustration: Influence of Loss function selection on epidemiological results

A City-Village example:

<div class="columns-2">

Consider two Loss functions which differ in the way they weigh the prediction error:

- *Quadratic loss function*: $\mathcal{L}_I = \big(y-\hat{f}(x)\big)' I \big(y-\hat{f}(x)\big)$
- *Precisioned quadratic loss function*: $\mathcal{L}_{\Sigma^{-1}} = \big(y-\hat{f}(x)\big)' \Sigma^{-1} \big(y-\hat{f}(x)\big)$

where $\Sigma^{-1}$ is the inverse of the errors variance-covariane matrix.
...
ggplot(data = coord, aes(x=x,y=y)) + geom_point(size = 1, alpha = 0.5) + 
  ...
  annotate("text", label = c("City","Village"),
           x = c(0.4,0.8), y = c(0.9,0.35), size = 5, colour = "darkred") +
...
Under $\mathcal{L}_I$ the model tend to be **more accurate** in locations where there are many **similar observations** (usually populated areas), **in the price of** higher errors in locations with **fewer observation**
...
So, which Loss function to chose? <span style="color:darkred">It depends on **what the goal is!**</span>
...
$$ \underset{t_i\in \mathcal{I}^T,t_j\in \mathcal{I}^V} {\arg\min} \left \{ |t_i-t_j| \right \} >h>0$$
...
ui <- fluidPage(
        ...
        server <- function(input, output) {
            ...
            output$distPlot <- renderPlot({
              ...
              coord_fixed() + xlim(0,1) + ylim(0,1) + 
              annotate("text", label = c("City","Village"),
                       x = c(0.4,0.8), y = c(0.9,0.3), size = 7, colour = "darkred") +
              theme(axis.title.x=element_blank(),
                    axis.text.x=element_blank(),
                    axis.ticks.x=element_blank(),
                    axis.title.y=element_blank(),
...
- Because ignoring it lead to inefficient estimates and <span style="color:darkred"> **bad predictions** at the geographic stage</span>.

<br />

- Also, Prediction errors may be spatio-temporally coralated
    + Predictions serve as **epidemiological covariates**
    + $\Rightarrow$ This might lead to **measurement errors**
    + $\qquad \Rightarrow$ <span style="color:darkred"> **Biased epidemiological results** </span> and erroneous conclusions
...
Advantages of the GLS/FGLS framework:

- <span style="color:darkred"> FGLS is the **minimizer** of the the **Precisioned quadratic loss function** </span>  (Mahalanobis norm) of the residual vector: $\hat{\beta}_{FGLS} =\underset{\beta}{\arg\min} \left \{ (y-X\beta)^{-1} \hat{\Sigma}^{-1} (y-X\beta) \right \}.$
- The GLS can be considered as an estimation method that <span style="color:darkred"> **de-correlate the scale** </span> of the OLS errors.
...
- Regression models differ by the definition of the **dependence structure** through the **covariance matrix** of the residuals terms. Hence, <span style="color:darkred"> GLS **includes as its special** cases various specific models, </span> such as the LME.
...
### How to estimate the Errors Variance-covariance Matrix?
...
<span style="color:darkred"> **Fixed in Space and Varying in Time:** </span>
...
<span style="color:darkred"> **Fixed in Time and Varying in Space:** </span>
...
<span style="color:darkred"> **Varying in Space and Time:** </span> We start with **Separable** covariance function (matrices **Kronecker product**).  **Nonseparable** functions will be studied further.
...
## The Implication of Estimation procedure on Epidemiological results - Demonstration
...
server <- function(input, output) {
    ...
    output$distPlot <- renderPlot({
        ...
        plot1 <- ggplot(melt(Temp[N:1,1:N]), aes(Var1,Var2, fill=value)) +
        ...
        plot3 <- ggplot(melt(Spat[(S:1),(1:S)]), aes(Var1,Var2, fill=value)) +
        ...
        plot8 <- ggplot(data = df, aes(y = pm.train, x = aod.train)) + geom_point(alpha = 0.5) +
            ...
            labs(title = "Geog. Model: pm~aod", x = "aod")
...
- <span style="color:darkred"> Apply **GLS** </span>
- Take advantage of some recent <span style="color:darkred"> **methodological and software developments** </span> that address the challenges of large scale data
...
The GLS **reduces the problem of model fitting** from a general optimization problem to the problem of <span style="color:darkred"> **solving a system of linear equations** </span>:
...
where: $\small \Sigma$'s Cholesky's decomposition is $\small \Sigma = L \Lambda L'$; $\small \Sigma^{-1} = PP'$; $\small P = L^{-1} \Lambda^{-\frac{1}{2}}$
 ...
 - The GLS setting allows to <span style="color:darkred"> harness a **very rich literature** </span> that explores methods for solving such problems in **large data**.

 - It also allows to <span style="color:darkred"> **control computational difficulty** </span> through the decision on the error covariance matrix, by so, to **balance** between prediction **accuracy and computational complexity**.
...
- Sparse data is more <span style="color:darkred">**easily compressed**</span>, and can be efficiently representated.
- Sparse matrix algorithms <span style="color:darkred">**allow faster computation**</span> by avoiding arithmetic operations on zero elements
...
## Computational Challenges - Memory Efficiency
 ...
 - To overcome this hurdle, we suggest to use <span style="color:darkred"> **External Memory Algorithms (EMA)** </span>: storing the data on the local storage (HD, SSD, etc.), and processing one chunk of it at a time in RAM
...
## Computational Challenges - Parallel Computing
 ...
 - Data sets are **splitted** into "chunks" and then the analysis is performed by <span style="color:darkred"> **multiple machines in parallel** </span>
...
ui <- fluidPage(
        ...
        server <- function(input, output) {
            ...
            output$distPlot <- renderPlot({
              ...
              coord <- cbind(x = c(rnorm(city, 0.25, 0.1), rnorm(village, 0.75, 0.05)),
                             y = c(rnorm(city, 0.75, 0.1), rnorm(village, 0.25, 0.05)))
              
              pm <- rnorm(city+village, mean = c(rep(0.5+(b),city),rep(0.5-(b),village)), 
                          sd = rep(0.1,city+village))
              
              d <- data.frame(coord, pm, reg = c(rep("A",city),rep("B",village)))
              
              ggplot(d, aes(x = x, y = y,colour = pm)) +
                geom_point(size = 3) + coord_fixed() +
                scale_color_gradient(low = "green", high = "red") + 
                xlim(0,1) + ylim(0,1) +
                annotate("text", label = c("A","B"),
                         x = c(0.25,0.75), y = c(0.75,0.25), size = 6)
              
              #gg2 <- ggplot(d, aes(x = reg, y = pm)) +
               # geom_boxplot() + ylab("")
